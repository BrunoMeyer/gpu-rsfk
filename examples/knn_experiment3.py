...
if __name__ == "__main__":
    ...
    tmp_real_indices = real_indices[:,1:]
    points_id, points_id_nb_count = np.unique(np.array(tmp_real_indices.reshape(-1,1)),return_counts=True)
    fig, axs = plt.subplots(1, 1, sharey=True, tight_layout=True)
    axs.hist(points_id_nb_count, bins=int(len(set(points_id_nb_count))/1))
    
    print(len(points_id_nb_count))
    points_id, points_id_nb_count = np.unique(points_id_nb_count,return_counts=True)
    
    max_idx = np.argmax(points_id_nb_count)
    print(points_id[max_idx], points_id_nb_count[max_idx])
    smaller_idx = np.argsort(points_id)[:50]
    print(points_id[smaller_idx], points_id_nb_count[smaller_idx])

    # axs.bar(points_id, points_id_nb_count)
    plt.show(fig)

    print(tmp_real_indices)
    ...
    graph = []
    for i in range(len(tmp_real_indices)):
        for j in tmp_real_indices[i]:
            graph.append((i,j))
    ...
    if TEST_RPFK:
        ...
        # for nnef in [3]:
            ...
            for n_trees in parameter_list:
                ...
                print("")
                indices = indices[:,1:]
                points_id, points_id_nb_count = np.unique(np.array(indices.reshape(-1,1)),return_counts=True)
                fig, axs = plt.subplots(1, 1, sharey=True, tight_layout=True)
                axs.hist(points_id_nb_count, bins=int(len(set(points_id_nb_count))/1))
