...
void assign_label(float* dataset, uint dataset_size, 
    ...
    for(uint ii = warpIdx*WARP_SIZE+blockIdx.x*N_WARPS*WARP_SIZE; ii < dataset_size; ii += N_WARPS*N_BLOCKS*WARP_SIZE){
        // GLOBAL FILTER
                    ...
                    if(min_update_count > 1){
                        ...
                        if(oldlabel != new_nearest){
                            ...
                            for(uint l=laneIdx; l < dim; l+=WARP_SIZE){
                                ...
                                new_centroids[new_nearest*dim+l] += dataset[i*dim+l];
                            }
                            for(uint l=laneIdx; l < dim; l+=WARP_SIZE){
                                // atomicAdd(&new_centroids[nearest_cent*dim+l], dataset[i*dim+l]);
                                new_centroids[oldlabel*dim+l] -= dataset[i*dim+l];
                            }
                        }
                    }
                }
