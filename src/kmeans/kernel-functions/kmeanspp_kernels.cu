#ifndef KMEANSPP_KERNELS_CU
#define KMEANSPP_KERNELS_CU
...
void append_centroid_kmeanspp(float* dataset, uint dataset_size, 
    ...
    float* sh_max_dist = reinterpret_cast<float*>(smem);
    uint*  sh_next_cent = reinterpret_cast<uint*>(
        sh_max_dist + blockDim.x
    );
