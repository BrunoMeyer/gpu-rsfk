...
main_arch75: src/rpfk.cu
	nvcc -O3 -gencode=arch=compute_75,code=sm_75 -std=c++11 src/rpfk.cu -o rpfk

python_arch75: src/python/pymodule_ext.cu
	nvcc -O3 -gencode=arch=compute_75,code=sm_75 -std=c++11 --compiler-options '-fPIC' -o python/gpu_rpfk/librpfk.so --shared src/python/pymodule_ext.cu
